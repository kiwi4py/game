...
MAXHEALTH = 8        # how much health the player starts with
...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, L_SQUIR_IMG, R_SQUIR_IMG, GRASSIMAGES, R_SHRIMP_IMG, \
        L_SHRIMP_IMG, R_FISH_IMG, L_FISH_IMG, R_SQUID_IMG, L_SQUID_IMG, L_SHELL_IMG, R_SHELL_IMG, \
        L_TURTLE_IMG, L_GOLDFISH_IMG, L_DIAMOND_IMG
    ...
    BASICFONT = pygame.font.SysFont('simhei', 32)

    # load the image files
    L_SQUIR_IMG = pygame.image.load('shark.png')              #
    R_SQUIR_IMG = pygame.transform.flip(L_SQUIR_IMG, True, False)

    L_SHRIMP_IMG = pygame.image.load('shrimp.png')              #
    R_SHRIMP_IMG = pygame.transform.flip(L_SHRIMP_IMG, True, False)

    L_SQUID_IMG = pygame.image.load('squid.png')
    L_SHELL_IMG = pygame.image.load('shell.png')
    L_FISH_IMG = pygame.image.load('fish.png')
    L_TURTLE_IMG = pygame.image.load('turtle.png')
    L_GOLDFISH_IMG = pygame.image.load('goldfish.png')
...
def runGame():
    ...
    gameOverSurf = BASICFONT.render(u'ÓÎÏ·½áÊø', True, WHITE)
    ...
    while True: # main game loop
        ...
        for sObj in squirrelObjs:
            ...
            if random.randint(0, 99) < DIRCHANGEFREQ:
                ...
                if sObj['movex'] > 0: # faces right
                    ...
                    if animal % 3 == 0:
                        sObj['surface'] = pygame.transform.scale(R_SHRIMP_IMG, (sObj['width'], sObj['height']))
                    elif animal % 5 == 0:
                        sObj['surface'] = pygame.transform.scale(L_SHRIMP_IMG, (sObj['width'], sObj['height']))
                    elif animal % 7 == 0:
                        sObj['surface'] = pygame.transform.scale(L_FISH_IMG, (sObj['width'], sObj['height']))
                    elif animal % 2 == 0:
                        sObj['surface'] = pygame.transform.scale(L_SQUID_IMG, (sObj['width'], sObj['height']))
                    elif animal % 11 == 0:
                        sObj['surface'] = pygame.transform.scale(L_SHELL_IMG, (sObj['width'], sObj['height']))
                    elif animal % 13 == 0:
                        sObj['surface'] = pygame.transform.scale(L_TURTLE_IMG, (sObj['width'], sObj['height']))
                    elif animal % 17 == 0:
                        sObj['surface'] = pygame.transform.scale(L_GOLDFISH_IMG, (sObj['width'], sObj['height']))
                else: # faces left
                    sObj['surface'] = pygame.transform.scale(L_SHRIMP_IMG, (sObj['width'], sObj['height']))
        ...
        for i in range(len(diamondObjs) - 1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, diamondObjs[i]):
                del diamondObjs[i]
        ...
        while len(diamondObjs) < NUMDIAMONDS:
            diamondObjs.append(makeNewDiamond(camerax, cameray))
        ...
        # draw diamond
        for diamondObj in diamondObjs:
            diamondObj['rect'] = pygame.Rect( (diamondObj['x'] - camerax,
                                                diamondObj['y'] - cameray,
                                                diamondObj['width'],
                                                diamondObj['height']))
            DISPLAYSURF.blit(diamondObj['surface'], diamondObj['rect'])
        ...
        healthSurf = BASICFONT.render(str(playerObj['health']), True, WHITE)
        healthRect = healthSurf.get_rect()
        healthRect.center = (WINWIDTH - 20, WINHEIGHT - 20)
        ...
        if not gameOverMode:
            ...
            for i in range(len(diamondObjs)-1, -1, -1):
                diamondObj = diamondObjs[i]
                if 'rect' in diamondObj and playerObj['rect'].colliderect(diamondObj['rect']):
                    playerObj['health'] += 1
                    makeSound()
...
def makeNewDiamond(camerax, cameray):
    diamonds = {}
    generalSize = random.randint(5, 25)
    multiplier = random.randint(1, 3)
    diamonds['width'] = (generalSize + random.randint(0, 10)) * multiplier
    diamonds['height'] = (generalSize + random.randint(0, 10)) * multiplier
    diamonds['x'], diamonds['y'] = getRandomOffCameraPos(camerax, cameray, diamonds['width'], diamonds['height'])
    diamonds['movex'] = getRandomVelocity()
    diamonds['movey'] = getRandomVelocity()
    diamonds['surface'] = pygame.transform.scale(L_DIAMOND_IMG, (diamonds['width'], diamonds['height']))
    return diamonds
